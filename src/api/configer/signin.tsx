...
export default async function signin(req: request, res: response, parameters: any): Promise<IResult> {
        ...
        memberInfo.signinTimes = (new Date().getTime()) - (memberInfo.signinDate || new Date()).getTime() < 2592000000
                ...
                : 1
        memberInfo.signinDate = new Date() // 登录时间
        memberInfo.signinAll = memberInfo.signinAll + 1 // 字段信息
        memberInfo.lastIp = memberInfo.signinIP // 最后登录IP地址
        memberInfo.lastTime = memberInfo.lastTime // 最后登录时间
        memberInfo.signinIP = req.IPAddress() // 登录IP地址
        memberInfo.signinTime = new Date() // 登录时间

        // 记录登录设备
        // if (parameters.deviceID) memberInfo.deviceID = parameters.deviceID
